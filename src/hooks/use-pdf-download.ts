...
export interface UsePdfDownloadReturn {
  ...
  handleDownloadPdf: (analysisResult: AnalysisResult | null, toast: (opts: { title: string; description?: string; variant?: "default" | "destructive" }) => void) => Promise<void>;
}

export function usePdfDownload(): UsePdfDownloadReturn {
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);

  const handleDownloadPdf = async (analysisResult: AnalysisResult | null, toast: (opts: { title: string; description?: string; variant?: "default" | "destructive" }) => void) => {
    if (!analysisResult) {
    ...
    try {
      const [{ saveAs }, { generateSingleAnalysisPdfReportAction }] = await Promise.all([
        import('file-saver'),
        import('@/app/actions')
      ]);
